...
import { store, categoriasProducto, categoriasServicio, addProducto, updateProducto, deleteProducto, addServicio, updateServicio, deleteServicio, getEstadoProducto, totalStock, parseMoney, formatMoney, type Producto, type Servicio } from '../store';
...
const formServicioVacio: ServicioForm = { descripcion: '', categoria: '', precio: '', empleado: '', notas: '' };
...
export default defineComponent({
  ...
  setup() {
    ...
    const guardarProducto = () => {
      ...
      store.sucursales.forEach((s) => { stockNum[s.id] = Number(form.value.stockPorSucursal[s.id]) || 0; });
      const data = {
        nombre: form.value.nombre,
        tipo: form.value.tipo || 'Motor',
        stockPorSucursal: stockNum,
        precio: formatMoney(Number(form.value.precio) || 0),
        proveedor: form.value.proveedor,
        descripcion: form.value.descripcion,
      };
      if (productoEditando.value) updateProducto(productoEditando.value, data);
    ...
    const abrirModalServicio = (servicio?: Servicio) => {
      if (servicio) {
        formServicio.value = { descripcion: servicio.descripcion, categoria: servicio.categoria, precio: String(servicio.precio), empleado: servicio.empleado, notas: servicio.notas ?? '' };
    ...
    const guardarServicio = () => {
      ...
      const data = {
        ...
        empleado: formServicio.value.empleado || 'Cualquier mecánico',
        notas: formServicio.value.notas,
      };
      if (servicioEditando.value) updateServicio(servicioEditando.value, data);
      ...
      return (
        <div class="space-y-5">
          ...
          {pestana.value === 'productos' ? (
            ...
            <div class="rounded-2xl shadow-sm overflow-hidden" style={{ background: '#fff', border: '1px solid rgba(15,23,42,0.06)' }}>
              <div class="overflow-x-auto"><table class="w-full"><thead><tr style={{ background: '#FFF7ED', borderBottom: '1px solid #FDBA74' }}>{['Código', 'Descripción del servicio', 'Categoría', 'Precio', 'Responsable', ''].map((h) => <th key={h} class="text-left px-4 py-3 text-xs font-semibold" style={{ color: '#C2410C' }}>{h}</th>)}</tr></thead><tbody>{serviciosPagina.map((s, i) => <tr key={s.id} style={{ borderBottom: '1px solid #FFF7ED', background: i % 2 === 0 ? '#fff' : '#FFFBF5' }}><td class="px-4 py-3 text-xs font-mono font-semibold" style={{ color: '#FB923C' }}>{s.id}</td><td class="px-4 py-3"><div class="flex items-center gap-2"><div class="w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0" style={{ background: '#FFF7ED' }}><Wrench size={13} style={{ color: '#FB923C' }} /></div><span class="text-sm font-semibold" style={{ color: '#0F172A' }}>{s.descripcion}</span></div></td><td class="px-4 py-3"><span class="px-2.5 py-1 rounded-full text-xs font-semibold" style={{ background: '#FFF7ED', color: '#C2410C' }}>{s.categoria}</span></td><td class="px-4 py-3 text-sm font-bold" style={{ color: '#FB923C' }}>{formatMoney(s.precio)}</td><td class="px-4 py-3 text-xs" style={{ color: '#64748B' }}>{s.empleado}</td><td class="px-4 py-3"><div class="flex items-center gap-1"><button onClick={() => abrirModalServicio(s)} class="w-7 h-7 rounded-lg flex items-center justify-center transition-colors hover:bg-orange-50"><Edit2 size={13} style={{ color: '#FB923C' }} /></button><button onClick={() => confirm('¿Estás seguro de eliminar este servicio?') && deleteServicio(s.id)} class="w-7 h-7 rounded-lg flex items-center justify-center transition-colors hover:bg-red-50"><Trash2 size={13} style={{ color: '#F87171' }} /></button></div></td></tr>)}</tbody></table></div>
